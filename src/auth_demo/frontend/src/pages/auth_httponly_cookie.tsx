...
export default function AuthHTTPOnlyCookie() {
    const onLogin = async ({ email, password } : {
        email: string, password: string,
    }) => {
        const res = await axios.post(`${API_BASE_URL}/by-cookie/auth/login`, {
            email, password,
        }, {
    ...
    const fetchEmailList = async () => {
        ...
        const res = await axios.get(`${API_BASE_URL}/by-cookie/users`, {
            withCredentials: true,
        });

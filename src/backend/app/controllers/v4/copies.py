...
from .services import copy_service as service
...
class Collection(Resource):
    ...
    @h.returns(
        ...
        collection_links=lambda content: [] if len(content) == 0 else [url_for( "v4.copies_collection", titleId=content[0]["titleId"])],
    )
    ...
    @api.doc(description="Thêm bản sao mới cho một đầu sách.", security="Bearer")
    @api.expect(copy_create_dto)
    @h.returns(
        copy_dto,
        self_links=lambda content: [url_for("v4.copies_item", titleId=content["titleId"], copyId=content["id"])],
        collection_links=lambda content: [url_for("v4.copies_collection", titleId=content["titleId"])],
    )
    @jwt_required()
...
class Item(Resource):
    ...
    @api.doc(description="Sửa toàn bộ copy, theo ID", security="Bearer")
    ...
    @api.doc(description="Sửa một phần copy, theo ID", security="Bearer")
    ...
    @api.doc(description="Xóa copy, theo ID", security="Bearer")
    @h.returns(
        api.model("empty", {}),
        self_links=lambda content: [],
        collection_links=lambda content: [url_for("v4.copies_collection", titleId=content["titleId"])],

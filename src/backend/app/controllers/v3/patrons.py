...
patron_dto = api.clone("Patron", patron_create_dto, {
    ...
    "isPremium": fields.Boolean(readonly=True, description="Là true nếu patron này là premium member của thư viện (có ít nhất 1 payment ủng hộ)")
})
...
from .services import patron_service as service
...
class Collection(Resource):
    ...
    @h.returns(
        ...
        collection_links=lambda content: [url_for("v3.patrons_collection")],
    )
    @jwt_required()
    def get(self):
    ...
    @h.returns(
        patron_dto,
        self_links=lambda content: [url_for("v3.patrons_item", patronId=content["id"])],
        collection_links=lambda content: [url_for("v3.patrons_collection")],
    )
    @jwt_required()
    def post(self):

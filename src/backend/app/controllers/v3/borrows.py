...
from .services import borrow_service as service
...
class Collection(Resource):
    ...
    @api.doc(description="Lấy danh sách tất cả các borrows, có pagination. Có thể lọc theo patronId.", security="Bearer")
    @api.expect(get_collection_qp)
    @h.returns(
        borrow_dto,
        as_list=True,
        self_links=lambda content: [],
        collection_links=lambda content: [url_for("v3.borrows_collection")],
    )
    @jwt_required()
    def get(self):
    ...
    @api.doc(description="Thêm borrow mới.", security="Bearer")
    @api.expect(borrow_create_dto)
    @h.returns(
        borrow_dto,
        self_links=lambda content: [url_for("v3.borrows_item", borrowId=content["id"])],
        collection_links=lambda content: [url_for("v3.borrows_collection")],
    )
    @jwt_required()
    def post(self):
...
class Item(Resource):
    ...
    @api.doc(description="Lấy borrow theo ID", security="Bearer")
    @h.returns(
        borrow_dto,
        self_links=lambda content: [url_for("v3.borrows_item", borrowId=content["id"])],
        collection_links=lambda content: [url_for("v3.borrows_collection")],
    ...
    @api.doc(description="Sửa một phần borrow, theo ID", security="Bearer")
    ...
    @api.doc(description="Xóa borrow, theo ID", security="Bearer")
    @h.returns(
        api.model("empty", {}),
        self_links=lambda content: [],
        collection_links=lambda content: [url_for("v3.borrows_collection")],
